...
//SOLICITUD PARA TRAER VIDEOJUEGOS DE LA API
const infoApi = async () => {
  let url = `https://api.rawg.io/api/games?key=6bcf7f7f25be4251b01417ff0c30ad7a`;
  let videojuegos = [];
  try {
    for (let i = 0; i < 5; i++) {
...
const nameApi = async (name) => {
  const infoSearch = await axios.get(
    `https://api.rawg.io/api/games?search=${name}&key=6bcf7f7f25be4251b01417ff0c30ad7a`
  );
...
const idApi = async (id) => {
  try {
    const rtaApi = await axios.get(
      `https://api.rawg.io/api/games/${id}?key=6bcf7f7f25be4251b01417ff0c30ad7a`
    );
    if (rtaApi) {
